...
export function BoletimAdmin() {
  ...
  const { dados, buscarListaAlunos, deleteBoletim } = useBoletimAdmin();
  ...
  const alunosFiltrados = useMemo(() => {
    ...
    const boletinsFiltrados = dados.filter((item) => {
      const nome = item.aluno?.alunoNome?.toLowerCase()?.trim() || "";
    ...
    return boletinsFiltrados.filter(
      (item, index, array) =>
        array.findIndex((b) => b.alunoId === item.alunoId) === index,
    );
  }, [dados, busca]);
    ...
    <View style={{ flex: 1 }}>
        ...
        {alunosFiltrados.length === 0 ? (
          ...
          alunosFiltrados.map((item) => (
            <CarteiraItem
              key={item.alunoId}
              icon="account-school"
              title={item.aluno?.alunoNome || "Aluno sem nome"}
      ...
      <ConfirmDialog
        ...
        description={`Deseja excluir todos os boletins de ${
          alunosFiltrados.find((a) => a.alunoId === selectedId)?.aluno.alunoNome
        }?`}
